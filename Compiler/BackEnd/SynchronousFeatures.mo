...
algorithm
  outDAE := match inDAE
    ...
    case (BackendDAE.DAE({syst}, shared)) guard(not Flags.isSet(Flags.NO_PARTITIONING) and BackendDAEUtil.isInitializationDAE(shared))
      then clockPartitioning1(syst, shared);
    // TODO: Improve support for partitioned systems of equations
    case (BackendDAE.DAE({syst}, shared)) guard(not Flags.isSet(Flags.NO_PARTITIONING) and BackendDAEUtil.isInitializationDAE(shared)) equation
...
algorithm
  ...
  (outExp, outTpl) := match inExp
    local
      ...
      DAE.Exp e;
      Real intvl;
    case DAE.CLKCONST(DAE.BOOLEAN_CLOCK(e, intvl))
      equation
        ({e}, newEqs, newVars, cnt) = substExp({e}, newEqs, newVars, cnt);
      then
        (DAE.CLKCONST(DAE.BOOLEAN_CLOCK(e, intvl)), (newEqs, newVars, cnt));
