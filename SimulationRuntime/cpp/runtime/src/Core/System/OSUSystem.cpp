...
void  OSUSystem::initialize()
{
  ...
  jm_status_enu_t instantiateModelStatus = fmi2_import_instantiate(_osu_me->instance, _osu_name.c_str(), fmi2_model_exchange, resource_location.string().c_str(), fmi2_false);
  if (instantiateModelStatus == jm_status_error) {
    _osu_me->solving_mode = osi_none_mode;
    const char* log_str = jm_log_level_to_string((jm_log_level_enu_t)instantiateModelStatus);
    std::string error = std::string("fmi2InstantiateModel failed with status :") + std::string(log_str);
    throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
  }
  /* allocate event info*/
  _osu_me->event_info = (fmi2_event_info_t*)malloc(sizeof(fmi2_event_info_t));
  _osu_me->solving_mode = osi_instantiated_mode;

  //get variable dimensions
  _dimContinuousStates = fmi2_import_get_number_of_continuous_states(_osu_me->instance);
  _dimRHS = _dimContinuousStates;
  _dimZeroFunc = fmi2_import_get_number_of_event_indicators(_osu_me->instance);
  ...
  fmi2_import_setup_experiment(_osu_me->instance, false, 0.0, 0.0, false, 0.0);
  _osu_me->solving_mode = osi_instantiated_mode;
  fmi2_status_t status = fmi2_import_enter_initialization_mode(_osu_me->instance);
  if (status != fmi1_status_ok && status != fmi1_status_warning)
  {
    const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
    std::string error = std::string("fmi2EnterInitializationMode failed with status :") + std::string(log_str);
    throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
  }
  _osu_me->solving_mode = osi_initialization_mode;
  ...
  _osu_me->solving_mode = osi_event_mode;
  if (status != fmi1_status_ok && status != fmi1_status_warning)
  {
    const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
    std::string error = std::string("ffmi2ExitInitializationMode failed with status  :") + std::string(log_str);
    throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
  }
  _instantiated = true;
  ...
  while (eventInfo->newDiscreteStatesNeeded && !eventInfo->terminateSimulation && !(iter++ > 100))
  {
    status = fmi2_import_new_discrete_states(_osu_me->instance, eventInfo);
    if (status != fmi1_status_ok && status != fmi1_status_warning) {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2EnterEventMode failed with status  :") + std::string(log_str);
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
  }
  if (eventInfo->newDiscreteStatesNeeded && !eventInfo->terminateSimulation)
    throw  ModelicaSimulationError(MODEL_EQ_SYSTEM, "eventFMUUpdate failed: Number of event iterations exeeded");
  ...
  _osu_me->solving_mode = osi_continuousTime_mode;
  saveAll();

    if(getGlobalSettings()->getOutputPointType()!= OPT_NONE)
   {
      _writeOutput = getSimObjects()->LoadWriter(_dimReal+_dimInteger+_dimBoolean).lock();
      _writeOutput->init();
     _writeOutput->clear();
   }
}
...
void  OSUSystem::initializeMemory()
{
  ...
  fmi2_import_variable_list_t* vl = fmi2_import_get_variable_list(_osu_me->instance, 0);
    ...
    for (i = 0; i < nv; i++)
  {
    ...
    if (var)
    {
      ...
      if (bt == fmi2_base_type_real)
      {
        ...
        isParameter = addValueReference(var, _real_out_vars_vr, _real_param_vars_vr,_dimReal);
                _dimReal++;

      }
      else if (bt == fmi2_base_type_int)
      {
        isParameter = addValueReference(var, _int_out_vars_vr, _int_param_vars_vr,_dimInteger);
                _dimInteger++;
      ...
      else if (bt == fmi2_base_type_bool)
      {
        ...
        isParameter = addValueReference(var, _bool_out_vars_vr, _bool_param_vars_vr,_dimBoolean);
                 _dimBoolean++;
      ...
      {
        isParameter = addValueReference(var, _string_out_vars_vr, _string_param_vars_vr,_dimString);
                 _dimString++;
    ...
    {
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, "Intialisation of value references failed");
    }

  }
  fmi2_import_free_variable_list(vl);
  _simVars = _simObjects->LoadSimVars(_modelName, _dimReal, _dimInteger, _dimBoolean, _dimString, _dimReal + _dimInteger + _dimBoolean + _dimString, _dimContinuousStates, -1).lock();
 ...
 *  \details Details
 */
bool OSUSystem::addValueReference(fmi2_import_variable_t* v, out_vars_vr_t& output_value_references, out_vars_vr_t& param_value_references,
                                   unsigned int var_idx)
{
  fmi2_causality_enu_t causality = fmi2_import_get_causality(v);
  size_t vr = fmi2_import_get_variable_vr(v);
  if ((causality == fmi2_causality_enu_parameter) || (causality == fmi2_causality_enu_calculated_parameter))
  {
...
void OSUSystem::initializeResultOutputVars()
{
      ...
      for(out_vars_vr_t::iterator iter = _real_out_vars_vr.begin(); iter!=_real_out_vars_vr.end();iter++)
      {
          fmi2_import_variable_t* v =  fmi2_import_get_variable_by_vr(_osu_me->instance,fmi2_base_type_real,get<0>(*iter));
    ...
    //add real parameter to writeoutput structure
    for (out_vars_vr_t::iterator iter = _real_param_vars_vr.begin(); iter != _real_param_vars_vr.end(); iter++)
    {
      fmi2_import_variable_t* v = fmi2_import_get_variable_by_vr(_osu_me->instance, fmi2_base_type_real, get<0>(*iter));
      string name = string(fmi2_import_get_variable_name(v));
      const char* descripton_cstr = fmi2_import_get_variable_description(v);
    ...
    //add boolean output variables to writeoutput structure
    for (out_vars_vr_t::iterator iter = _bool_out_vars_vr.begin(); iter != _bool_out_vars_vr.end(); iter++)
    {
      fmi2_import_variable_t* v = fmi2_import_get_variable_by_vr(_osu_me->instance, fmi2_base_type_bool, get<0>(*iter));
      string name = string(fmi2_import_get_variable_name(v));
      const char* descripton_cstr = fmi2_import_get_variable_description(v);
      string descripton;
      if (descripton_cstr)
    ...
    //add boolean parameter to writeoutput structure
    for (out_vars_vr_t::iterator iter = _bool_param_vars_vr.begin(); iter != _bool_param_vars_vr.end(); iter++)
    {
      fmi2_import_variable_t* v = fmi2_import_get_variable_by_vr(_osu_me->instance, fmi2_base_type_bool, get<0>(*iter));
      string name = string(fmi2_import_get_variable_name(v));
      const char* descripton_cstr = fmi2_import_get_variable_description(v);
    ...
    //add integer output variables to writeoutput structure
    for (out_vars_vr_t::iterator iter = _int_out_vars_vr.begin(); iter != _int_out_vars_vr.end(); iter++)
    {
      fmi2_import_variable_t* v = fmi2_import_get_variable_by_vr(_osu_me->instance, fmi2_base_type_int, get<0>(*iter));
      string name = string(fmi2_import_get_variable_name(v));
      const char* descripton_cstr = fmi2_import_get_variable_description(v);
      string descripton;
      if (descripton_cstr)
    ...
    //add integer parameter to writeoutput structure
    for (out_vars_vr_t::iterator iter = _int_param_vars_vr.begin(); iter != _int_param_vars_vr.end(); iter++)
    {
      fmi2_import_variable_t* v = fmi2_import_get_variable_by_vr(_osu_me->instance, fmi2_base_type_int, get<0>(*iter));
      string name = string(fmi2_import_get_variable_name(v));
      const char* descripton_cstr = fmi2_import_get_variable_description(v);
...
{
  if ((_osu_me->solving_mode == osi_continuousTime_mode))
  {
    ...
    status = fmi2_import_enter_continuous_time_mode(_osu_me->instance);
    if (status != fmi1_status_ok && status != fmi2_status_warning) {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2EnterEventMode failed with status  :") + std::string(log_str);
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
    ...
    status = fmi2_import_get_event_indicators(_osu_me->instance, (fmi2_real_t*)_zeroVal, _dimZeroFunc);
    if ((status != fmi1_status_ok) && (status != fmi1_status_warning) && (status != fmi1_status_discard))
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2GetEventIndicators failed with status ::") + std::string(log_str);
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
...
{
    if ((_osu_me->solving_mode == osi_continuousTime_mode) || (_osu_me->solving_mode == osi_event_mode))
  {
        //write inputs
        //read outputs
...
{
  throw ModelicaSimulationError(MATH_FUNCTION, "evaluateAll is for osu system not supported");
}
...
{
  if ((_instantiated) && (_osu_me->solving_mode == osi_continuousTime_mode))
  {
    ...
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2SetTime failed with status  :") + std::string(log_str);
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
  }
}
...
{
    fmi2_event_info_t *eventInfo = _osu_me->event_info;
    fmi2_status_t status = fmi2_import_new_discrete_states(_osu_me->instance, eventInfo);
...
void OSUSystem::getContinuousStates(double* z)
{
  if ((_osu_me->solving_mode == osi_continuousTime_mode) || (_osu_me->solving_mode == osi_event_mode))
  {
    ...
    fmi2_status_t status = fmi2_import_get_continuous_states(_osu_me->instance, (fmi2_real_t*)z, _dimContinuousStates);
    if (status != fmi1_status_ok && status != fmi1_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2GetContinuousStates failed with status :") + std::string(log_str);
      throw  ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
...
void OSUSystem::getNominalStates(double* z)
{
  ...
  if (_osu_me->solving_mode == osi_continuousTime_mode)
  {
    fmi2_status_t status = fmi2_import_get_nominals_of_continuous_states(_osu_me->instance, (fmi2_real_t*)z, _dimContinuousStates);
    if (status != fmi1_status_ok && status != fmi1_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2SetContinuousStates failed with status :") + std::string(log_str);
      throw  ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
  }
...
void OSUSystem::setContinuousStates(const double* z)
{
  if (_osu_me->solving_mode == osi_continuousTime_mode)
  {
    fmi2_status_t status = fmi2_import_set_continuous_states(_osu_me->instance, (fmi2_real_t*)z, _dimContinuousStates);
    if (status != fmi1_status_ok && status != fmi1_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2SetContinuousStates failed with status :") + std::string(log_str);
...
void OSUSystem::getRHS(double* f)
{
  if ((_osu_me->solving_mode == osi_continuousTime_mode) || (_osu_me->solving_mode == osi_event_mode))
  {
    ...
    fmi2_status_t status = fmi2_import_get_derivatives(_osu_me->instance, (fmi2_real_t*)f, _dimRHS);
    if (status != fmi1_status_ok && status != fmi1_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2GetDerivatives failed with status  :") + std::string(log_str);
      throw ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
  }
}
...
bool OSUSystem::isStepEvent(double time)
{
  if (_osu_me->solving_mode == osi_continuousTime_mode)
  {
    ...
    fmi2_status_t status = fmi2_import_completed_integrator_step(_osu_me->instance, fmi2_true, &callEventUpdate, &terminateSimulation);
    if (status != fmi1_status_ok && status != fmi1_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2CompletedIntegratorStep failed with status :") + std::string(log_str);
      throw  ModelicaSimulationError(MODEL_EQ_SYSTEM, error);
    }
...
void OSUSystem::getZeroFunc(double* f)
{
  if ((_osu_me->solving_mode == osi_continuousTime_mode) || (_osu_me->solving_mode == osi_event_mode))
  {
    ...
    fmi2_status_t status = fmi2_import_get_event_indicators(_osu_me->instance, (fmi2_real_t*)_zeroVal, _dimZeroFunc);
    if ((status != fmi1_status_ok) && (status != fmi1_status_warning) && (status != fmi1_status_discard))
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("fmi2GetEventIndicators failed with status ::") + std::string(log_str);
...
void OSUSystem::writeOutput(const IWriteOutput::OUTPUT command)
{
  ...
  {
     const all_names_t outputVarNames = make_tuple(_real_vars.ourputVarNames,_int_vars.ourputVarNames,_bool_vars.ourputVarNames);
     const all_names_t outputVarDescription = make_tuple(_real_vars.ourputVarDescription,_int_vars.ourputVarDescription,_bool_vars.ourputVarDescription);
     const all_names_t parameterVarNames =  make_tuple(_real_vars.parameterNames,_int_vars.parameterNames,_bool_vars.parameterNames);
     const all_names_t parameterVarDescription =  make_tuple(_real_vars.parameterDescription,_int_vars.parameterDescription,_bool_vars.parameterDescription);
     _writeOutput->write(outputVarNames,outputVarDescription,parameterVarNames,parameterVarDescription);
      const all_vars_t params = make_tuple(_real_vars.outputParams,_int_vars.outputParams,_bool_vars.outputParams);
  ...
  else
  {
      ...
      all_vars_time_t all_vars = make_tuple(_real_vars.outputVars,_int_vars.outputVars,_bool_vars.outputVars,_simTime);
      neg_all_vars_t neg_all_vars =      make_tuple(_real_vars.negateOutputVars,_int_vars.negateOutputVars,_bool_vars.negateOutputVars);
...
void OSUSystem::getReal(double* z)
{
  ...
  {
    fmi2_value_reference_t* value_reference_list = &(get<0>(_real_out_vars_vr[0]));
        fmi2_status_t status = fmi2_import_get_real(_osu_me->instance, value_reference_list, _real_out_vars_vr.size(), (fmi2_real_t*)z);

    if (status != fmi2_status_ok && status != fmi2_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("getReal failed with status  :") + std::string(log_str);
...
void OSUSystem::getInteger(int* z)
{
  ...
  {
    fmi2_value_reference_t* value_reference_list = &(get<0>(_int_out_vars_vr[0]));
        fmi2_status_t status = fmi2_import_get_integer(_osu_me->instance, value_reference_list, _int_out_vars_vr.size(), (fmi2_integer_t*)z);

    if (status != fmi2_status_ok && status != fmi2_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("getInteger failed with status  :") + std::string(log_str);
      throw std::runtime_error(error);
    }
  }
...
void OSUSystem::getBoolean(bool* z)
{
  ...
  {
        fmi2_value_reference_t* value_reference_list = &(get<0>(_bool_out_vars_vr[0]));
    fmi2_status_t status = fmi2_import_get_boolean(_osu_me->instance,value_reference_list, _bool_out_vars_vr.size(), (fmi2_boolean_t*)z);

    if (status != fmi2_status_ok && status != fmi2_status_warning)
    {
      const char* log_str = fmi2_status_to_string((fmi2_status_t)status);
      std::string error = std::string("getBoolean failed with status  :") + std::string(log_str);
      throw std::runtime_error(error);
    }
  }
...
{
  throw ModelicaSimulationError(MATH_FUNCTION, "isJacobianSparse is for osu system not supported");
}

/* DAE residuals is empty */
void OSUSystem::getResidual(double* f)
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getResidual is for osu system not supported");
}
void OSUSystem::setAlgebraicDAEVars(const double* y)
{
  throw ModelicaSimulationError(MATH_FUNCTION, "setAlgebraicDAEVars is for osu system not supported");
}
/* get algebraic variables */
void OSUSystem::getAlgebraicDAEVars(double* y)
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getAlgebraicDAEVars is for osu system not supported");
}


bool OSUSystem::isAnalyticJacobianGenerated(){
  throw ModelicaSimulationError(MATH_FUNCTION, "isAnalyticJacobianGenerated is for osu system not supported");
}

const matrix_t& OSUSystem::getJacobian()
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getJacobian is for osu system not supported");
}
...
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getSparseJacobian is for osu system not supported");
}
...
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getStateSetJacobian is for osu system not supported");
}
sparsematrix_t& OSUSystem::getStateSetSparseJacobian(unsigned int index)
{

  throw ModelicaSimulationError(MATH_FUNCTION, "getStateSetSparseJacobian is for osu system not supported");

}

void OSUSystem::getAColorOfColumn(int* aSparsePatternColorCols, int size)
{
  throw ModelicaSimulationError(MATH_FUNCTION, "getAColorOfColumn is for osu system not supported");
}
